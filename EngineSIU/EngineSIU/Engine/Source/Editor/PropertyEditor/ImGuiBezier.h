...
namespace ImGui
{
    ...
    int Bezier(const char *label, float P[5]) {
        ...
        float vecx = Window->DC.CursorPos.x + Canvas.x;
        float vecy = Window->DC.CursorPos.y + Canvas.y;
        ImRect bb(Window->DC.CursorPos, ImVec2(vecx, vecy));
        ItemSize(bb);
        if (!ItemAdd(bb, NULL))
            return changed;

        const ImGuiID id = Window->GetID(label);
        hovered |= 0 != ItemHoverable(ImRect(bb.Min, ImVec2(bb.Min.x + avail, bb.Min.y + dim)), id, 0);
        ...
        // control points: 2 lines and 2 circles
        {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                pos[i].x = P[i * 2 + 0] * (bb.Max.x - bb.Min.x) + bb.Min.x;
                pos[i].y = (1 - P[i * 2 + 1]) * (bb.Max.y - bb.Min.y) + bb.Min.y;
                // pos[i] = ImVec2(P[i * 2 + 0] * (bb.Max.x - bb.Min.x), (1 - P[i * 2 + 1]) * (bb.Max.y - bb.Min.y)) + bb.Min;
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            // ImVec2 p0 = ImVec2(evalx * (bb.Max.x - bb.Min.x), bb.Max.y - bb.Min.y) + bb.Min;
            ImVec2 p0;
            p0.x = evalx * (bb.Max.x - bb.Min.x) + bb.Min.x;
            p0.y = bb.Max.y;
            // ImVec2 p1 = ImVec2(0, 1 - evaly) * (bb.Max - bb.Min) + bb.Min;
            ImVec2 p1;
            p1.x = bb.Min.x;
            p1.y = (1 - evaly) * (bb.Max.y - bb.Min.y) + bb.Min.y;
            // ImVec2 p2 = ImVec2(evalx, 1 - evaly) * (bb.Max - bb.Min) + bb.Min;
            ImVec2 p2;
            p2.x = evalx * (bb.Max.x - bb.Min.x) + bb.Min.x;
            p2.y = (1 - evaly) * (bb.Max.y - bb.Min.y) + bb.Min.y;
        ...
        ImVec2 p1;
        p1.x = P[0] * (bb.Max.x - bb.Min.x) + bb.Min.x;
        p1.y = (1 - P[1]) * (bb.Max.y - bb.Min.y) + bb.Min.y;
        // ImVec2 p2 = ImVec2(P[2], 1 - P[3]) * (bb.Max - bb.Min) + bb.Min;
        ImVec2 p2;
        p2.x = P[2] * (bb.Max.x - bb.Min.x) + bb.Min.x;
        p2.y = (1 - P[3]) * (bb.Max.y - bb.Min.y) + bb.Min.y;
